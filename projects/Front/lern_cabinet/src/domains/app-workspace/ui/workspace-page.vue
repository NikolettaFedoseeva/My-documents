...
const flashcardData: QuestionData = {
  ...
  answer:
    'ref() оборачивает любой тип данных в объект с полем .value.\nreactive() работает ТОЛЬКО с объектами и создает глубокую прокси-обертку.',
}
...
const onRateFlashcard = (payload: { rating: string }) => {
  quizRatingResult.value = `Ответ сохранён: ${payload.rating === 'know' ? 'Знаю 🟢' : 'Повторить 🔴'}`
}
  ...
  <div class="workspace-page" :class="{ 'workspace-page--focus': isFocusMode }">
    ...
    <header v-if="!isFocusMode" class="workspace-header">
      <div class="workspace-header__left">
        <div class="workspace-header__brand">
          <span class="workspace-header__icon">📖</span>
          <span class="workspace-header__title">Lern Codex Workspace</span>
        </div>
        <UiBadge variant="primary" dot>ВЕРСИЯ 1.0</UiBadge>
      </div>

      <div class="workspace-header__center">
        <div class="workspace-header__user">
          <UiAvatar name="Анна Смирнова" status="online" size="md" />
          <div class="workspace-header__user-info">
            <span class="workspace-header__user-name">Анна Смирнова</span>
            <span class="workspace-header__user-role">Исследователь • Уровень 4</span>
          </div>
        </div>
      </div>
    ...
    <div v-if="!isFocusMode" class="workspace-progress-strip">
      <UiProgressBar
        ...
        variant="primary"
        :label="`Общий прогресс прохождения курса: ${overallProgress}%`"
        show-label
        animated
      />
    </div>
    ...
    <div v-else class="focus-exit-bar">
      <span>✨ Режим полной концентрации (Focus Mode)</span>
      <UiButton variant="ghost" size="sm" @click="toggleFocus">Выйти из Focus Mode ✕</UiButton>
    </div>
      ...
      <section v-if="currentView === 'search'" class="workspace-view">
        <UiCard variant="glass" padding="lg">
          <div class="workspace-search-header">
            <h2>🧭 Справочный Каталог Знаний</h2>
            <p>Выберите тему или воспользуйтесь живым поиском</p>
          </div>

          <div class="workspace-search-input">
            <UiInput v-model="searchQuery" placeholder="Поиск по темам и разделам..." clearable>
          ...
          <div class="workspace-topics-grid">
            ...
            >
              <div class="workspace-topic-card__header">
                ...
                <span class="workspace-topic-card__time">⏱ {{ item.readTime }}</span>
              </div>
              ...
              <div class="workspace-topic-card__footer">
                ...
                <UiButton variant="secondary" size="sm">Читать →</UiButton>
              </div>
            </div>
          </div>
        </UiCard>
      ...
      <section v-if="currentView === 'tree'" class="workspace-view">
        <UiCard variant="glass" padding="lg">
          <div class="workspace-tree-header">
            <h2>🗺 Иерархическая Карта Знаний</h2>
            <p>Нажмите на любую тему для перехода к её изучению</p>
          </div>
          ...
          <div class="workspace-tree-list">
            ...
            >
              <div class="tree-row__title">
                <span class="tree-row__icon">{{ topicItem.isCompleted ? '✓' : '📄' }}</span>
              ...
              <div class="tree-row__right">
                ...
                <UiButton variant="secondary" size="sm">Открыть</UiButton>
              </div>
            </div>
          </div>
        </UiCard>
        ...
        <UiCard variant="parchment" padding="lg">
          <div class="reader-header">
            ...
            <div class="reader-meta">
              <UiBadge variant="secondary">⏱ {{ currentTopic.readTime }}</UiBadge>
              <UiProgressBar :value="currentTopic.progress" size="sm" style="width: 140px" />
              <span>Освоено {{ currentTopic.progress }}%</span>
            </div>
          </div>
          ...
          <div class="reader-body">
            ...
            <div v-if="currentTopic.codeSnippet" class="code-box">
              <div class="code-box__header">Фрагмент кода</div>
              <pre><code>{{ currentTopic.codeSnippet }}</code></pre>
            </div>
          </div>

          <div class="reader-footer">
            <UiButton variant="secondary" @click="setView('tree')">← Назад к карте</UiButton>
            <UiButton variant="primary" @click="setView('quiz')">Перейти к самопроверке 🎯 →</UiButton>
          </div>
        </UiCard>
      </section>
      ...
      <section v-if="currentView === 'quiz'" class="workspace-view">
        <UiCard variant="glass" padding="lg">
          <div class="quiz-header">
            <h2>🎯 Самопроверка: {{ currentTopic.title }}</h2>
            <p>Ответьте на вопросы и проверьте память</p>
          </div>

          <UiAlert v-if="currentTopic.isCompleted" variant="success" style="margin-bottom: 1.5rem">
            Тема уже освоена на 100%! Вы можете пройти проверку повторно для закрепления.
          </UiAlert>

          <UiQuestion
            v-model="selectedQuizOption"
            question="Когда рекомендуется применять данный механизм?"
            :options="quizOptions"
            correct-id="opt-b"
